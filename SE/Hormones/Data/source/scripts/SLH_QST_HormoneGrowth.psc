...
Function Maintenance()
	...
	If (StorageUtil.GetIntValue(none, "_SLH_iHormonesSleepInit")==0)
		...
		debugNotification("[Hormones] Changes paused until next sleep.")
	Else
		debugNotification("[Hormones] Changes enabled.")		
	Endif
...
; Game.GetPlayer().ModActorValue("Stamina", 10.0)
	...
	debugTrace("[SLH] OnSleep: StaminaRate (modified) = " + PlayerActor.GetActorValue("StaminaRate"))
	debugTrace("[SLH] OnSleep: HealRate (modified) = " + PlayerActor.GetActorValue("HealRate"))
	debugTrace("[SLH] OnSleep: MagickaRate (modified) = " + PlayerActor.GetActorValue("MagickaRate"))
...
Function debugNotification(string traceMsg)
	; for debug messages in game
	if (StorageUtil.GetIntValue(none, "_SLH_debugTraceON")==1)
		Debug.Notification(traceMsg)
	endif
endFunction
